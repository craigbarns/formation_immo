...
export function FormationShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="formation-canvas min-h-screen bg-slate-50 dark:bg-[#030712] text-slate-900 dark:text-white">
      <header className="sticky top-0 z-50 border-b border-slate-200 dark:border-white/10 bg-white/95 dark:bg-[#030712]/80 backdrop-blur-xl shadow-sm dark:shadow-2xl">
        <div className="mx-auto max-w-6xl px-4 py-2 border-b border-slate-100 dark:border-white/[0.05]">
          <p className="text-center text-xs font-medium leading-snug text-slate-500 dark:text-white/70 sm:text-left">
        ...
        <div className="relative z-10 mx-auto flex max-w-6xl flex-col gap-4 px-4 pb-4 pt-2 lg:flex-row lg:items-center lg:justify-between lg:gap-6">
          <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2 sm:gap-x-5">
            <Link
              href="/formation"
              className="group flex shrink-0 items-center gap-3 leading-tight text-slate-900 dark:text-white transition hover:text-brand-gold dark:hover:text-brand-gold"
            >
              <div
              ...
              <span className="min-w-0">
                ...
                <span className="block truncate text-lg font-black tracking-tight text-slate-900 dark:text-white sm:text-xl">
                  Formation 42 h
                </span>
              </span>
            </Link>
            <div className="flex min-w-0 items-center border-l border-slate-200 dark:border-white/15 pl-3 sm:pl-5">
              <XPBar />
            </div>
          </div>
          <nav
            className="flex flex-wrap items-center justify-center gap-1.5 border-t border-slate-100 dark:border-white/10 pt-3 text-sm lg:justify-end lg:border-t-0 lg:pt-0"
          ...
          >
            ...
            <MagneticButton strength={0.2}>
              <button
                ...
                className="flex items-center gap-2 rounded-full border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-4 py-2 text-sm font-bold text-slate-700 dark:text-white/80 outline-none transition hover:bg-slate-50 dark:hover:bg-white/15 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-white/20"
              >
                <Search className="w-4 h-4" />
                <span className="hidden sm:inline">Rechercher</span>
                <kbd className="hidden md:inline-flex items-center px-1.5 py-0.5 text-xs font-mono bg-slate-100 dark:bg-white/10 rounded border border-slate-200 dark:border-white/10">
                  ⌘K
                </kbd>
              </button>
            </MagneticButton>
      ...
      <footer className="border-t border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-[#020617] py-16">
        <div className="mx-auto flex max-w-6xl flex-col gap-10 px-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="max-w-md space-y-4 text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-2">
              ...
              <p className="text-sm font-black uppercase tracking-widest text-slate-800 dark:text-white">FORMATION 42 H</p>
            </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-3 text-xs font-bold uppercase tracking-wider sm:justify-end">
              <Link
                ...
                className="text-slate-600 dark:text-white/70 transition hover:text-brand-gold"
              >
                Fiches visuelles
              </Link>
              <Link
            ...
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-xs text-slate-500 dark:text-white/60 sm:justify-end">
              <span>Formation certifiante ALUR 2026</span>
              <span aria-hidden>·</span>
              <Link href="/mentions-legales" className="hover:text-slate-700 dark:hover:text-white/80 transition">Mentions légales</Link>
              <span aria-hidden>·</span>
              <Link href="/cgv" className="hover:text-slate-700 dark:hover:text-white/80 transition">CGV</Link>
              <span aria-hidden>·</span>
              <a href="mailto:contact@formation42h.fr" className="hover:text-slate-700 dark:hover:text-white/80 transition">Contact</a>
            </div>
          </div>
        </div>
      </footer>
...
function NavItem({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="rounded-full px-3.5 py-2 text-sm font-bold text-slate-600 dark:text-white/70 transition hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white focus-ring-brand-dark"
    >
      {children}
    </Link>
  );
...
function MobileNavItem({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link
      href={href}
      className="flex flex-col items-center gap-1 px-3 py-1 text-slate-500 dark:text-white/60 transition-colors hover:text-brand-gold"
    >
