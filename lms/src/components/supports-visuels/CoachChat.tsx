...
export function CoachChat({ moduleSlug = "supports-visuels", lessonSlug = "fiches", lessonTitle = "Supports visuels & fiches opérationnelles" }: CoachChatProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/coach", {
        ...
        body: JSON.stringify({ messages: allMessages.map(m => ({ role: m.role, content: m.content })), moduleSlug, lessonSlug, lessonTitle }),
      });
  ...
  // Sync ref for useEffect below
  useEffect(() => { handleSendRef.current = handleSend; }, [handleSend]);

  useEffect(() => {
    if (!isListening && input.trim() && !isLoading && autoSpeak) {
      const timer = setTimeout(() => handleSendRef.current?.(), 600);
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 flex w-[22rem] max-w-[calc(100vw-3rem)] flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 p-4 max-h-80">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-2 ${m.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                ...
                <div className="flex max-w-[80%] flex-col gap-1">
                  <div className={`rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${m.role === "user" ? "bg-brand-navy text-white rounded-br-sm" : "bg-zinc-100 text-zinc-800 rounded-bl-sm"}`}>{cleanForDisplay(m.content)}</div>
                  {m.role === "assistant" && (
            ...
            {streaming && <div className="flex gap-2 flex-row"><div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-gold/15 text-brand-gold"><Bot className="h-3.5 w-3.5" /></div><div className="max-w-[80%] rounded-2xl rounded-bl-sm bg-zinc-100 px-3.5 py-2.5 text-sm text-zinc-800">{cleanForDisplay(streaming)}<span className="inline-block h-2 w-2 animate-pulse rounded-full bg-brand-navy/40 ml-1" /></div></div>}
          ...
          <div className="flex items-center gap-2 border-t border-zinc-100 p-3">
            ...
            <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSendRef.current?.()} placeholder={isListening ? "Écoute..." : "Pose ta question..."} disabled={isListening} className="flex-1 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 text-sm outline-none focus:border-brand-navy/30 focus:bg-white disabled:opacity-50" />
