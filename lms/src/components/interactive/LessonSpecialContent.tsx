...
const MapPrixImmo = dynamic(
  ...
  { ssr: false, loading: () => <div className="h-[500px] animate-pulse rounded-[2.5rem] bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10" /> }
);

const MarcheImmoChart = dynamic(
  () => import("@/components/charts/MarcheImmoChart").then((m) => ({ default: m.MarcheImmoChart })),
  { ssr: false, loading: () => <div className="h-[400px] animate-pulse rounded-[2.5rem] bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10" /> }
);
  ...
  if (moduleSlug === "transaction" && lessonSlug === "estimation") {
    return (
      <div className="space-y-12">
        <section>
          <h2 className="lesson-block-title text-slate-900 dark:text-white">
            <span className="lesson-block-title-line bg-brand-gold/30" aria-hidden />
            Carte des prix immobiliers France 2026
          </h2>
          <p className="mb-6 text-base text-slate-600 dark:text-white/50 font-medium italic">
          ...
          <div className="rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-white/10 shadow-2xl">
            <MapPrixImmo />
          </div>
        </section>
        ...
        <section>
          ...
          <div className="rounded-[2.5rem] border border-slate-200 dark:border-white/10 bg-white dark:bg-[#070d18] p-8 shadow-2xl">
            <MarcheImmoChart />
          </div>
        </section>
      </div>
